...
LRESULT _stdcall WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)		// оконная процедура принимает и обрабатывает все сообщения, отправленные окну
{
	...
	case WM_RBUTTONDOWN:
		{
		secretPrint.MoveTo(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			InvalidateRect(hWnd, nullptr, false);
			return 0;
		}
	case WM_LBUTTONDOWN:
		{
		secretPrint.StartDragging(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			return 0;
		}
	case WM_MOUSEMOVE:
		{
			if (secretPrint.IsDragging())
			{
				secretPrint.Drag(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
				InvalidateRect(hWnd, nullptr, false);
			}
			return 0;
		}
	...
	case WM_MOUSEWHEEL:
		...
		if (cntrl) {
			...
			secretPrint.Resize(GET_WHEEL_DELTA_WPARAM(wParam), P.x, P.y);
			InvalidateRect(hWnd, nullptr, false);
			return 0;
		}
		}
